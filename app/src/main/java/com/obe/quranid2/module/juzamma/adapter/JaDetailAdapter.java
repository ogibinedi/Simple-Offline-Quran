...
public class JaDetailAdapter extends RecyclerView.Adapter<JaDetailViewHolder> {
    private final ArrayList<JuzammaSurah> surahList;
    private final ArrayList<JuzammaSurah> surahListFull;
    private final Context mContext;

    public JaDetailAdapter(ArrayList<JuzammaSurah> surahList, Context mContext) {
    ...
    @NonNull
    @Override
    public JaDetailViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_ayah, parent, false);
        return new JaDetailViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull JaDetailViewHolder holder, int position) {
        holder.setJaData(surahList.get(position));

        if (position %2 == 1){
            holder.cvJaDetail.setCardBackgroundColor(mContext.getResources().getColor(R.color.orange_200));
        }else {
            holder.cvJaDetail.setCardBackgroundColor(mContext.getResources().getColor(R.color.white));
        }

        holder.cvJaDetail.setOnClickListener(view -> {
    ...
    private final Filter ayahFilter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            List<JuzammaSurah> filterList = new ArrayList<>();
            if (constraint == null || constraint.length() == 0){
                filterList.addAll(surahListFull);
            }else {
                String filterPattern = constraint.toString().toLowerCase().trim();
                for (JuzammaSurah surah : surahListFull){
        ...
        protected void publishResults(CharSequence constraint, FilterResults results) {
            surahList.clear();
            surahList.addAll((List<JuzammaSurah>) results.values);
