package com.obe.quranid2.module.juzamma.activities;
...
import com.obe.quranid2.module.juzamma.adapter.JaDetailAdapter;
...
public class JaDetailActivity extends AppCompatActivity {
    private final ArrayList<JuzammaSurah> surahs = new ArrayList<>();
    private RecyclerView recyclerView;
    private JaDetailAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ja_detail);
    ...
    private void initToolbar() {
        Toolbar toolbar = findViewById(R.id.toolbar_jad);
        toolbar.setTitle("( "+getIntent().getStringExtra("surahNumber")+" ) " +getIntent().getStringExtra("surahText"));
        String about_surah =getIntent().getStringExtra("type") +", "+getIntent().getStringExtra("totalAyah");
        toolbar.setSubtitle(about_surah);
        ...
        service.execute(() -> {
            ...
            try {
                ...
                for (int i=0; i < surahPilihanArray.length(); i++){
                    JuzammaSurah surah = new JuzammaSurah();
            ...
            runOnUiThread(() -> {
                final String JA_PREFS = getIntent().getStringExtra("surahNumber");
                recyclerView = findViewById(R.id.rv_jad);
                recyclerView.setHasFixedSize(true);
                layoutManager  = new LinearLayoutManager(getApplicationContext());
                recyclerView.setLayoutManager(layoutManager);
                RecyclerView.SmoothScroller smoothScroller = new LinearSmoothScroller(JaDetailActivity.this){
                    @Override
                ...
                adapter = new JaDetailAdapter(surahs, JaDetailActivity.this);
                recyclerView.setAdapter(adapter);
                Handler handler = new Handler();
                handler.postDelayed(() -> pDialog.cancel(), 1000);

                // menampilkan posisi terakhir list pada saat dibuka kembali
                SharedPreferences getPrefs = getApplicationContext().getSharedPreferences(JA_PREFS, MODE_PRIVATE);
    ...
    private String loadJSONFromAssets() {
        String json;
        String rawFile = "ja/"+getIntent().getStringExtra("surahNumber")+".json";
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        if (item.getItemId() == R.id.action_goto){
            ...
            edtGoto.setWidth(50);
            edtGoto.setHint("1-"+getIntent().getStringExtra("totalAyah"));
            ...
            builder.setView(edtGoto)
                    ...
                    .setMessage("Masukan nomor ayat antara 1-"+getIntent().getStringExtra("totalAyah"))
