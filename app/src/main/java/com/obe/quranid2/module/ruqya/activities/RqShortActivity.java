...
public class RqShortActivity extends AppCompatActivity {
    ...
    private static final String RQ_SHORT_PREFS = "RqShort";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_rq_short);
    ...
    private void initToolbar() {
        ...
        toolbar.setTitle("Bacaan Ruqyah Pendek");
    ...
    private void getTheData(){
        ...
        service.execute(() -> {
            ...
            try {
                ...
                // Fetch JSOnArray named Rq Pendek
                JSONArray ruqyahShortArray = obj.getJSONArray("ayahs");
                // Getting dzikir list data
                for (int i=0; i < ruqyahShortArray.length(); i++){
                    Ruqyah ruqyah = new Ruqyah();
                    // Fetch single ruqyah data
                    JSONObject ruqyahDetail = ruqyahShortArray.getJSONObject(i);
            ...
            runOnUiThread(() -> {
                recyclerView = findViewById(R.id.rv_rq_short);
                recyclerView.setHasFixedSize(true);
                LinearLayoutManager layoutManager  = new LinearLayoutManager(getApplicationContext());
                recyclerView.setLayoutManager(layoutManager);
                RecyclerView.SmoothScroller smoothScroller = new LinearSmoothScroller(RqShortActivity.this){
                    @Override
                ...
                adapter = new RuqyahAdapter(ruqyahs, RqShortActivity.this);
                recyclerView.setAdapter(adapter);
                Handler handler = new Handler();
                handler.postDelayed(() -> pDialog.cancel(), 1000);

                // menampilkan posisi terakhir list pada saat dibuka kembali
                SharedPreferences getPrefs = getApplicationContext().getSharedPreferences(RQ_SHORT_PREFS, MODE_PRIVATE);
    ...
    private String loadJSONfromAssets() {
        ...
        try{
            InputStream is = getAssets().open("rq/rq_short.json");
